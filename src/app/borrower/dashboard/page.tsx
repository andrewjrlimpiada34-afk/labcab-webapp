...
// Component for the live countdown
...
export default function BorrowerDashboard() {
  ...
  const overdueCount = activeTransactions.filter(t => t.deadline && new Date(t.deadline) < currentTime).length;
    ...
    <DashboardLayout role="borrower">
      <div className="flex flex-col gap-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="flex items-center gap-3 bg-secondary/20 border border-border/50 p-4 rounded-2xl backdrop-blur-md h-fit">
            ...
            <div>
              ...
              <p className="text-sm font-mono font-bold text-white">{format(currentTime, "MMM dd, yyyy | HH:mm")}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                          ...
                          <TableCell className="text-muted-foreground font-medium">
                            <div className="space-y-1">
                              <div className="flex items-center gap-2 text-xs">
                                ...
                                <span className="font-mono">{tx.borrowTime ? format(tx.borrowTime.toDate(), "MMM dd, HH:mm") : "..."}</span>
                              </div>
                              <div className="flex items-center gap-2 text-xs">
                                <span className="w-12 text-[10px] uppercase opacity-40">End</span>
                                <span className="font-mono text-primary/80">{tx.deadline ? format(new Date(tx.deadline), "MMM dd, HH:mm") : "..."}</span>
                              </div>
                            </div>
                          </TableCell>
          ...
          {/* Side Panel: Notifications & History Summary */}
            ...
            <Card className="border-border/50 bg-card/10 backdrop-blur-3xl rounded-[2.5rem] overflow-hidden shadow-xl">
              ...
              <CardContent className="p-0">
                <div className="divide-y divide-border/30">
                  ...
                  <div className="p-6 hover:bg-white/5 transition-colors cursor-pointer group">
                    <div className="flex gap-4">
                      <div className="w-1 bg-accent rounded-full group-hover:w-1.5 transition-all" />
                      <div>
                        <p className="text-sm font-bold text-white mb-1">Return Confirmed</p>
                        <p className="text-xs text-muted-foreground leading-relaxed">Admin verified your return of 'Beaker (x2)'.</p>
                      </div>
                    </div>
                  </div>
                </div>
